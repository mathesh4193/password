...
export default function ResetPassword() {
  ...
  useEffect(() => {
    ...
    async function validate() {
      ...
      try {
        const res = await axios.get(`${process.env.REACT_APP_API || 'https://password-reset-backend-se7q.onrender.com/'}/api/auth/validate-reset-token`, {
          params: { token, email }
        });
        if (res.data.valid) {
          setValid(true);
  ...
  const handleSubmit = async e => {
    ...
    setSubmitting(true);
    try {
      await axios.post(`${process.env.REACT_APP_API || 'https://password-reset-backend-se7q.onrender.com/'}/api/auth/reset-password`, {
